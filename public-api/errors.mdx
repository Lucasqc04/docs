...
```bash
# ❌ Errado
curl -X POST https://public-api-prod.dominipay.com.br/api-public/payments
...
### Token antigo (rotação de token)

**Mensagem:** `Apenas o token mais recente é válido.`

**Causa:** A loja gerou um token novo e o token anterior deixou de ser aceito.

**Solução:**
- Gere um token novo no painel (`https://admin.dominipay.com.br/profile?tab=store`)
- Atualize sua integração para usar apenas o token ativo mais recente
...
| 401 | Apenas o token mais recente é válido. | Token antigo após rotação de token |
