...
* **Autenticidade** (o webhook foi realmente enviado pela Dominipay)
...
<Note>
Apesar da marca Dominipay, os headers técnicos de webhook atualmente usam o prefixo legado `X-Paguebit-*` para compatibilidade.
</Note>
...
| Header                 | Descrição                         |
| ---------------------- | --------------------------------- |
| `X-Dominipay-Signature` | Assinatura HMAC-SHA256 do payload |
| `X-Dominipay-Timestamp` | Timestamp Unix (segundos)         |
| `X-Dominipay-Event-Id`  | ID único do evento                |

> **Observação:** headers HTTP são *case-insensitive*.
> `X-Dominipay-Signature` e `x-dominipay-signature` são equivalentes.
...
4. Envia o resultado no header `X-Dominipay-Signature` (hexadecimal)
...
1. Leia os headers:
   ...
   * `X-Dominipay-Signature`
   * `X-Dominipay-Timestamp`
...
app.post(
  ...
  (req, res) => {
    const signature = req.headers['x-dominipay-signature'] as string;
    const timestamp = req.headers['x-dominipay-timestamp'] as string;
...
| `feeAmount`   | number? | Taxa aplicada (quando houver)   |
| `liquidValue` | number? | Valor líquido (quando houver)   |
...
| Header da assinatura   | `X-Dominipay-Signature`  |
| Header do timestamp    | `X-Dominipay-Timestamp`  |
